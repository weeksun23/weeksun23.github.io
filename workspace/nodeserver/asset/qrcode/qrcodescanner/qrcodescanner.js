(function(){
	...
	function QrcodeScanner(element,options){
		...
		this.options = mix({
			width : 150,
			height : 125,
			scanningSrc : 'qrcodescanner/scanning.html?' + (+new Date),
			//识别成功，返回识别结果
	...
	QrcodeScanner.prototype = {
		init : function(){
			...
			element.innerHTML = "<div style='width:" + options.width + 
				"px;height:" + options.height + "px;border: 1px solid #d2d2d2;background-color: #000;margin:0px auto;'>" +
						"<iframe style='width: 100%;height: 100%;' src='" + options.scanningSrc + 
